...
**尚未實測的部分**：`FAILED#<id>`（只驗證過 `BLOCKED#<id>` 這一種手動要求對方輸出特定標記的情境，`FAILED` 未重複驗證，但走的是同一套「要求對方直接輸出該字串」的驗證方法，判斷邏輯相同）、真正的 pane 消失、以及忙碌中搶送指令會發生什麼——這幾個仍是類推自 `../wmux/SKILL.md` 既有的通用提醒，不是本技能實測過的案例。
