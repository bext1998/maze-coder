# maze-coder — Pi Router

> 由 sync-adapters.sh 產生，請勿手動編輯。

先讀取 `.pi/maze-coder/HARNESS_ENGINEERING.md`，依 Host 與模型能力選擇最輕可用 Profile，再按需載入一份 Model Overlay。依意圖只載入一個最相關的公開技能；技能明示組合時才載入 internal skill，其他 references、templates 或 checklists 也只按指標讀取。
