...
## 邊界
不寫規格書、不實作、不寫程式碼、不產 PR、不替使用者決策；不在建圖 session 解決 issue；已畢業迷霧不退回，已關閉 issue 不重開。GitHub Issues 操作委派 `maze-github-cli`（Pi 路徑：`../../maze-coder/internal-skills/maze-github-cli/SKILL.md`）。
